...
```sql
SELECT * FROM( SELECT UNNEST(STRING_TO_ARRAY(country, ',')) AS country,COUNT(*) AS total_conten FROM netflix GROUP BY 1) AS t1 WHERE country IS NOT NULL ORDER BY total_content DESC LIMIT 5;
```
